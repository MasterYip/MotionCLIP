...
def read_single_sequence(split_name, dataset_name, folder, seq_name, body_models, target_fps, max_fps_dist,
    ...
    for subject in tqdm(subjects):
        ...
        for action in actions:
            ...
            seq_subj_action = seq_subj_action[:-13] + ".npz"  # remove _100_jpos.npz suffix
            ...
            frame_raw_text_labels = np.full(data['dof_positions'].shape[0], "", dtype=object)
            frame_proc_text_labels = np.full(data['dof_positions'].shape[0], "", dtype=object)
            frame_action_cat = np.full(data['dof_positions'].shape[0], "", dtype=object)
            ...
            if babel_dict['frame_ann'] is None:
                ...
                start_frame = 0
                end_frame = data['dof_positions'].shape[0]
            ...
            if target_fps is not None:
                mocap_framerate = float(data['fps'])
